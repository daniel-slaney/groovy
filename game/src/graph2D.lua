...
require 'src/Graph'
require 'src/Vector'
require 'src/graphgen'
local AABB = require 'src/AABB'

local function aabb( graph )
...
local function matchAABB( match )
	local xmin, xmax = math.huge, -math.huge
	local ymin, ymax = math.huge, -math.huge
...
local function nearest( graph1, graph2 )
...
local function connect( graph, rooms )
	local centres = {}
...
local function subdivide( graph, margin )
	local subs = {}
...
local function assignRoomsAndRelax( graph, theme, yield )
...
local function meanEdgeLength( graph )
	local totalLength = 0
	local count = 0
...
local function isSelfIntersecting( graph )
	local vertices = {}
	for vertex, _ in pairs(graph.vertices) do
		vertices[#vertices+1] = vertex
	end
...
local function aStar( graph, fromVertex, toVertex, vertexFilter )
...
local function randomWalk( graph )
	local vertexArray = table.keys(graph.vertices)

	local startIndex = math.random(1, #vertexArray)
	local startVertex = vertexArray[startIndex]
	local headVertex = startVertex

	local visited = { [startVertex] = true }
	local path = { startVertex }

	for i = 2, #vertexArray do
		local peers = graph.vertices[headVertex]
		local candidates = {}

		for peerVertex, edge in pairs(peers) do
			if not visited[peerVertex] then
				candidates[#candidates+1] = peerVertex
			end
		end
		...
		if #candidates == 0 then
			...
			headVertex = candidates[math.random(1, #candidates)]
			visited[headVertex] = true
			path[#path+1] = headVertex
		end

		-- local candidates = table.keys(peers)
		-- headVertex = candidates[math.random(1, #candidates)]

		-- if visited[headVertex] then
		-- 	break
		-- else
		-- 	visited[headVertex] = true
		-- 	path[#path+1] = headVertex
	...
	print('randomWalk', #vertexArray, #path)
...
return {
	...
	subdivide = subdivide,
	forceDraw = forceDraw,
	assignRoomsAndRelax = assignRoomsAndRelax,
